...
> [!NOTE]
> **預警資料同步 (`alerts`) 延後實作**：本階段優先完成週期健康紀錄的同步。預警資料（`alerts`）的本地正規化與同步流程將在預警分析模組完成後另行實作。目前 API payload 中保留 `"alerts": []` 空陣列作為擴展預留。
...
### 階段一：後端資料表與 API 實作
- [ ] **後端環境配置**：安裝 `msgpack` 與 `zstandard` 套件。
- [ ] **後端資料庫模型**：在 `server/app/models/periodic_health_record.py` 新建 SQLAlchemy 週期健康紀錄模型。
- [ ] **資料庫模型匯入**：在 `server/app/core/database.py` 的 `create_db_tables` 內匯入新模型以自動建表。
- [ ] **後端 Pydantic Schema**：定義 Request/Response 的驗證 Schema（包含 Base64 `raw_data_payload`）。
- [ ] **批次同步 API**：在 `server/app/api/sync.py` 實作 `POST /api/sync/batch` 路由、Base64 解碼與寫入 `BYTEA` 邏輯、交易控制與 `on_conflict_do_nothing`，並掛載至 `server/app/main.py`。

### 階段二：前端資料表與壓縮排程器實作
- [ ] **前端環境配置**：安裝 `@msgpack/msgpack` 與 `zstd-codec` 套件。
- [ ] **前端資料表建置**：在 `mobile-app/src/db/sqlite.ts` 新增 `CREATE_PERIODIC_HEALTH_RECORDS_TABLE_SQL` 語句。
- [ ] **壓縮引擎開發**：建立 `mobile-app/src/modules/health-simulation/engine/packer.ts`，實作數學聚合以及 MsgPack+ZSTD 二進制壓縮邏輯。
- [ ] **排程觸發器**：在前端建立一個 10 分鐘定期觸發打包壓縮的 Scheduler。
- [ ] **異常修補機制**：實作 Boot-up 未完整視窗掃描與向後合併處理。

### 階段三：前端同步機制與網路對接
- [ ] **同步客戶端 (SyncClient)**：建立 API 連線封裝，支援讀取 `pending` 資料並發送。
- [ ] **隨機偏移與連線監聽**：實作 `0 ~ 599 秒` 的隨機 Delay，並調用 `@capacitor/network` 以便在離線時自動暫緩同步。
- [ ] **本地狀態更新**：在同步成功後更新本地 SQLite 紀錄為 `synced`。

### 階段四：端到端 (E2E) 測試與驗證
- [ ] **正常路徑驗證**：運行模擬器，觀察 5 秒產生一次資料，10 分鐘成功打包並上傳至 FastAPI 後端。
- [ ] **斷網恢復測試**：在同步時切換網路為離線，驗證狀態維持 `pending`，並在重新連線後下一週期補傳。
- [ ] **冪等性測試**：模擬網路逾時導致的重傳，驗證 PostgreSQL 不會重複入庫。
